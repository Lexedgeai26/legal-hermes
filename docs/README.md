# LexEdge Developer Documentation

This folder contains product, architecture, operations, and implementation notes for the LexEdge Personal AI Assistant codebase.

LexEdge is built on the MIT-licensed Hermes Agent project by Nous Research:

- LexEdge source: https://github.com/Lexedgeai26/legal-hermes
- Product overview: https://lexedge.ai/agent/
- Upstream project: https://github.com/NousResearch/hermes-agent
- LexEdge website: https://www.lexedge.ai/

## Start Here

- [Desktop Installation for macOS and Windows](desktop-installation.md)
- [Introductory video recording guide for legal users](legal-user-intro-video-recording-guide.md)
- [Matter Workspaces](matter-workspaces.md)
- [Using LexEdge with n8n](n8n-guide.md)
- [Unified Online Installer implementation plan](unified-online-installer-implementation-plan.md)
- [Private AI Installer — Developer and Manual QA Guide](private-ai-installer-qa-guide.md)
- [Private AI Installer — Developer Build and Test Guide](private-ai-installer-developer-guide.md)
- [Unified Online Installer gap analysis and developer handoff](unified-online-installer-gap-analysis-and-developer-handoff.md)
- [Developer Handbook](developer/README.md)
- [Architecture Overview](developer/architecture.md)
- [Feature Extension Guide](developer/feature-extension-guide.md)
- [UI Development Guide](developer/ui-development.md)
- [AI, Agents, Skills, and Providers](developer/ai-agents-skills-providers.md)
- [Data, Security, and Enterprise Readiness](developer/data-security-enterprise.md)
- [Build, Release, and Installed App Runbook](developer/build-release-runbook.md)
- [Desktop Installer Builds for macOS and Windows](developer/desktop-installer-builds.md)
- [Roadmap for Future Enterprise Features](developer/enterprise-roadmap.md)

## Existing Product Docs

- [Lawyer onboarding wizard](lexedge-lawyer-onboarding-wizard.md)
- [WhatsApp legal messaging guide](lexedge-whatsapp-legal-messaging-guide.md)
- [Importing Claude skills — full guide](claude-skill-import.md)
- [Help docs](help-docs/)

## Support

- Public bugs and feature requests: https://github.com/Lexedgeai26/legal-hermes/issues
- Private security reports: https://github.com/Lexedgeai26/legal-hermes/security/advisories/new
