# ============================================================================
# LexEdge Hermes Agent Installer for Windows
# ============================================================================
...
$RepoUrlSsh = "git@github.com:Lexedgeai26/legal-hermes.git"
$RepoUrlHttps = "https://github.com/Lexedgeai26/legal-hermes.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|           * LexEdge Hermes Agent Installer              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  Private legal AI by LexEdge AI Labs Private Limited.   |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    # Nous Research stays credited as the upstream project author.
    Write-Host "Built on Hermes, an open source project by Nous Research."
    Write-Host ""
}
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/Lexedgeai26/legal-hermes/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/Lexedgeai26/legal-hermes/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/Lexedgeai26/legal-hermes/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Install-Desktop {
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\LexEdge AI.exe",
        "$desktopDir\release\win-arm64-unpacked\LexEdge AI.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no LexEdge AI.exe was found under $desktopDir\release\*-unpacked\"
    }
    ...
    try {
        ...
        foreach ($lnkPath in $targets) {
            try {
                ...
                $sc.Description = 'LexEdge Hermes Agent'
                $sc.Save()
