// LexEdge Hermes Agent Setup — process entrypoint. All logic lives in lib.rs so it can
...
// cmd window behind LexEdge-Hermes-Agent-Setup.exe on release builds.
//
...
fn main() {
    lexedge_hermes_agent_bootstrap_lib::run()
}
