...
fn main() {
    ...
    if let Some(c) = &commit {
        ...
        println!(
            "cargo:warning=lexedge-hermes-agent-bootstrap: pinning to commit {}",
            short(c)
        );
    }
    if let Some(b) = &branch {
        ...
        match &commit {
            Some(_) => println!("cargo:warning=lexedge-hermes-agent-bootstrap: pinning to branch {b}"),
            None => println!(
                "cargo:warning=lexedge-hermes-agent-bootstrap: following branch {b} HEAD (no commit pin; \
    ...
    if commit.is_none() && branch.is_none() {
        ...
        // build environment.
        println!(
            "cargo:warning=lexedge-hermes-agent-bootstrap: no pin resolved at build time; binary will fail at runtime without HERMES_SETUP_DEV_REPO_ROOT or runtime args"
        );
    }
    ...
    // -----------------------------------------------------------------
    // Tauri windows manifest. See lexedge-hermes-agent-setup.manifest for rationale —
    // declares level="asInvoker" so Windows's installer-detection
    // heuristic doesn't refuse to launch us without UAC elevation.
    // -----------------------------------------------------------------
    #[cfg(target_os = "windows")]
    let attrs = {
        let manifest = include_str!("lexedge-hermes-agent-setup.manifest");
